...
sys.path.append("/home/luoxinglong/meshflow")
...
from adm import UNet, MDS, MDC, MDC_little
...
class ADMEEMFlow(nn.Module):
    def __init__(self, config, groups=5, n_first_channels=5, out_mesh_size=False):
        ...
        self.unet = MDC_little(n_first_channels, n_first_channels)
        self.unet_sk = MDS(n_first_channels)
        ...
        self.decoder_1 = Decoder(65, groups)
        self.decoder_2 = Decoder(65, groups)
        self.decoder_3 = Decoder(65, groups)
    ...
    def load_flownet_model(self, freeze=True):
        states = torch.load("/home/luoxinglong/meshflow/a_best_checkpoint/EEMFlow_HREM_dt1.tar")
        state_dict = {}
        ...
        if(freeze):
            for name, param in self.named_parameters():
                ...
                if(('unet' in name) or ('unet_sk' in name)):
                    print("### {} is free!".format(name))
                    param.requires_grad = True
                else:
                    print("NO FREE: {} is freezing!".format(name))
    ...
    def forward(self, events1, events2):
        ...
        events_p1, events_p2 = self.image_padder.pad(events1, events2)

        events_p1 = events_p1.contiguous()
        events_p2 = events_p2.contiguous()

        unet_out = self.unet([events_p1, events_p2])
        # print(unet_out)

        # image1, image2 = self.unet_sk([events_p1, events_p2], [unet_out[0], unet_out[1]])
        ...
        # print("model forward completely")
...
def time_eval(model, batch_size=2, iters=32):  # 12
    ...
    tensor_tools.compute_model_size(model, images, images)
...
if __name__ == '__main__':
    ...
    config = ''
    model = ADMEEMFlow(config, groups=5, n_first_channels=5)
    time_eval(model)