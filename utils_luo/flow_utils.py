...
def mesh_warp_frame(frame, x_motion_mesh, y_motion_mesh, mesh_size=16):
    """
    ...
    def shift_flow_with_mirror_border(flow, offset_x, offset_y):
        H, W = flow.shape
        ...
        # 中心区域平移赋值
        shifted_flow[y_start:H, x_start:W] = flow[0:H - offset_y, 0:W - offset_x]

        # 边缘处理：左右镜像
        if offset_x > 0:
            shifted_flow[:, :4*offset_x] = shifted_flow[:, 4*offset_x:8*offset_x][:, ::-1]
            shifted_flow[:, W - 4*offset_x:] = shifted_flow[:, W - 8*offset_x:W - 4*offset_x][:, ::-1]

        # 边缘处理：上下镜像
        if offset_y > 0:
            shifted_flow[:offset_y, :] = shifted_flow[offset_y:2*offset_y, :][::-1, :]
            shifted_flow[H - offset_y:, :] = shifted_flow[H - 2*offset_y:H - offset_y, :][::-1, :]
        return shifted_flow

    # spreads motion over the mesh for the old_frame    
    mesh_cols, mesh_rows = frame.shape[1]//mesh_size, frame.shape[0]//mesh_size

    mesh_width = mesh_cols*mesh_size
    mesh_height = mesh_rows*mesh_size

    # define handles on mesh in x-direction
    map_x = np.zeros((mesh_height, mesh_width), np.float32)
    
    # define handles on mesh in y-direction
    map_y = np.zeros((mesh_height, mesh_width), np.float32)

    offect_x = mesh_rows//2
    offect_y = mesh_cols//2

    for i in range(mesh_size-1):
        for j in range(mesh_size-1):

            src = [[j*mesh_cols, i*mesh_rows],
                   [j*mesh_cols, (i+1)*mesh_rows],
                   [(j+1)*mesh_cols, i*mesh_rows],
                   [(j+1)*mesh_cols, (i+1)*mesh_rows]]
            src = np.asarray(src)
            
            dst = [[j*mesh_cols+x_motion_mesh[i, j], i*mesh_rows+y_motion_mesh[i, j]],
                   [j*mesh_cols+x_motion_mesh[i+1, j], (i+1)*mesh_rows+y_motion_mesh[i+1, j]],
                   [(j+1)*mesh_cols+x_motion_mesh[i, j+1], i*mesh_rows+y_motion_mesh[i, j+1]],
                   [(j+1)*mesh_cols+x_motion_mesh[i+1, j+1], (i+1)*mesh_rows+y_motion_mesh[i+1, j+1]]]
            dst = np.asarray(dst)
            H, _ = cv2.findHomography(src, dst, cv2.RANSAC)
            
            for k in range(mesh_rows*i, mesh_rows*(i+1)):
                for l in range(mesh_cols*j, mesh_cols*(j+1)):
    ...
    # for i in range(mesh_rows*x_motion_mesh.shape[0], map_x.shape[0]):
    #         map_x[i, :] = map_x[mesh_rows*x_motion_mesh.shape[0]-1, :]
    #         map_y[i, :] = map_y[mesh_rows*x_motion_mesh.shape[0]-1, :]
    
    # # repeat motion vectors for remaining frame in x-direction
    # for j in range(mesh_cols*x_motion_mesh.shape[1], map_x.shape[1]):
    #         map_x[:, j] = map_x[:, mesh_cols*x_motion_mesh.shape[0]-1]
    #         map_y[:, j] = map_y[:, mesh_cols*x_motion_mesh.shape[0]-1]
    
    # map_x_resize = cv2.resize(map_x, (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_LINEAR)
    # map_y_resize = cv2.resize(map_y, (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_LINEAR)

    map_x = shift_flow_with_mirror_border(map_x, offect_x, offect_y)
    map_y = shift_flow_with_mirror_border(map_y, offect_x, offect_y)

    # 创建一个网格，表示每个像素的位置
    x, y = np.meshgrid(np.arange(frame.shape[1]), np.arange(frame.shape[0]))

    # 计算新的坐标：原始坐标 + 光流的偏移
    new_x = x + map_x  # u 方向的偏移
    new_y = y + map_y  # v 方向的偏移

    # # 限制坐标值在图像范围内
    # new_x = np.clip(new_x, 0, width - 1)
    # new_y = np.clip(new_y, 0, height - 1)

    # 将新的坐标转换为 OpenCV 格式的 float32 类型
    map_x = new_x.astype(np.float32)
    map_y = new_y.astype(np.float32)

    # deforms mesh
    new_frame = cv2.remap(frame, map_x, map_y, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    return new_frame

def mesh_homo_vis(frame, x_motion_mesh, y_motion_mesh, mesh_size=16):
    """
    @param: frame is the current frame
    @param: x_motion_mesh is the motion_mesh to 
            be warped on frame along x-direction
