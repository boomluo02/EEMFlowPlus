        ...
        with torch.no_grad():
            ...
            for sequence in sequence_list:
                ...
                for batch_idx, batch in enumerate(self.data_loader):
                    ...
                    # batch_idx1 = int(batch['idx'].cpu().data.numpy()[0]) - 1
                    # istr = '{:05d} / {:05d}  AEE: {:2.6f}  meanAEE:{:2.6f} 1 - mean %AEE: {:.6f}'.format(batch_idx + 1, len(self.data_loader), AEE, AEE_sum / iters, 1.-percent_1_AEE_sum / iters)
                    istr = '{:s} {:05d} / {:05d}  AEE: {:2.6f}  meanAEE:{:2.6f} 3 - mean %AEE: {:.6f}'.format(sequence, batch_idx + 1, len(self.data_loader), AEE, AEE_sum / iters, 1.-percent_3_AEE_sum / iters)
                    print(istr)
                    ...
                    if visualize_map:
                        ...
                        if(warp_img):
                            # save_warp_root = "/home/luoxinglong/meshflow/vis_loader/align_image_spflow"
                            # os.makedirs(os.path.join(save_warp_root, sequence), exist_ok=True)
                            # save_warp_path = os.path.join(save_warp_root, sequence, '{:d}_{}_warped.png'.format(idx, self.args.model_name))
                            # if(os.path.exists(save_warp_path)):
                            #     continue
                            ...
                            # imageio.imwrite(save_warp_path, img2_new) 


                            save_warp_root = f"/home/luoxinglong/meshflow/vis_loader/pred_results_{self.args.model_name}"
                            os.makedirs(os.path.join(save_warp_root, sequence), exist_ok=True)
                            save_warp_path = os.path.join(save_warp_root, sequence, '{:d}_image2_warped.png'.format(idx))
                            save_pred_path = os.path.join(save_warp_root, sequence, '{:d}_meshflow_pred.flo'.format(idx))
                            if(os.path.exists(save_warp_path)):
                                continue
                            img1 = batch['img1'].cpu()[0].numpy().transpose(1,2,0)
                            img2 = batch['img2'].cpu()[0].numpy().transpose(1,2,0)
                            ...
                            # img2_new = mix_warp_image(img2_warp, img2)
                            # imageio.imwrite(save_warp_path, img2_new) 
                            cv2.imwrite(save_warp_path, img2_warp)
                            file_tools.flow_read_write.write_flo(flow_est, save_pred_path)
